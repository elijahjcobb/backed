...
import { TObjectStruct, TObject } from "typr";
...
export class HRequest<T = any> {
  ...
  public verifyPayloadAgainstTypeDefinition(types: TObjectStruct<T>): void {
    if (this.payloadObject === undefined)
      throw HError.init().code(400).msg("Payload undefined.").show();
    HServer.neon.log(types);
    HServer.neon.log(this.payloadObject);
    const x = TObject.follow(types);
    console.log(x);
    console.log(Object.keys(x));
    console.log(x.conforms(this.payloadObject));
    if (!TObject.follow(types).conforms(this.payloadObject))
